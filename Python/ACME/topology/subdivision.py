...
def __xmesh(T,scheme,iter=None):
    ...
    def repel(a,dim):
      ...
      for d in range(0,len(dim)):
        out = np.repeat(out,dim[d],axis=d)
      return out
    t = torch.t(T).cpu().numpy()
    M = np.identity(np.max(t)+1)
    if iter is not None:
        t = flatten(t)
        M = M[t]
        for i in range(0,iter):
            e = np.tile(scheme[0],(row(M)//row(T),1)) + repel(torch2numpy(indices(0,row(M)//row(T)-1,device='cpu')),(row(scheme[0]),2))*row(T)
            m = np.zeros((row(e),col(M)))
            for j in range(0,col(e)):
                m = m+M[e[:,j]]
            M = (1/col(e))*m
        t = transpose(np.reshape(torch2numpy(indices(0,row(M)-1,device='cpu')),(int(row(M)/3),3)))
...
def xquad(T,iter=None):
    ...
    k = 2
    l = 3
    scheme = (np.array([[i,i,i,i],[i,i,j,j],[i,j,k,l],[i,i,l,l],
...
def xtri2quad(T):
    ...
    j = 1
    k = 2
    scheme = (np.array([[i,i,i,i,i,i],[i,i,i,j,j,j],[i,i,j,j,k,k],[i,i,i,k,k,k],
