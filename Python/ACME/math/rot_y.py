...
def rot_y(theta,affine=False,device='cuda:0'):
    """
    Creates a 3D rotation matrix around y axis with the specified angle in radians
    ...
    R = axang2rotm(FloatTensor([0,1,0],device=device),theta)
    if affine:
        R = rotm2affine(R)
    return R
