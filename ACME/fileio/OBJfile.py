...
def export_OBJ(filename, Point=None, Normal=None, UV=None, Face=None, verbose=False):
    ...
    def writeDataFcn(fileID):
        ...
        if Face is not None:
            txt = (face_format(hasUV, hasN) + ' ') * row(Face)
            txt = 'f ' + txt[0:-1] + '\n'
            for f in repelem(torch.t(Face).cpu(), 1, 1 + hasUV + hasN):
                content += (txt.format(*tuple(f.numpy() + 1)))
...
def import_OBJ(filename, device='cuda:0', verbose=False):
    ...
    def readDataFcn(fileID):
        ...
        # read vertex data
        V = string2tensor('\n'.join(list(x for x in content if x[0:2] == 'v ')), prefix='v ', dtype=torch.float,
                          device=device)
        # read uv data
        UV = string2tensor('\n'.join(list(x for x in content if x[0:3] == 'vt ')), prefix='vt ', dtype=torch.float,
                           device=device)
        # read normal data
        N = string2tensor('\n'.join(list(x for x in content if x[0:3] == 'vn ')), prefix='vn ', dtype=torch.float,
                          device=device)
        # isolate face data
